...
function RecommendationsCarousel() {
  ...
  <h1 className="text-white text-center text-2xl md:text-5xl p-4 animate-slide-in-down p-5" style={{ textShadow: "2px 2px 4px rgba(0, 0, 0, 1.5)" }}>
  ...
  <div className="overflow-hidden">
    ...
    >
      {recommendations.map((recommendation) => (
        <div key={recommendation.nombre} className="testimonial-item">
          <div className="bg-white rounded-lg shadow-lg flex flex-col justify-center items-center gap-5 p-4">
            <div className="flex items-center justify-center">
              <div className="w-12 h-12 mr-4 rounded-full overflow-hidden">
                <img className="w-full h-full object-cover" src={recommendation.imagen} alt={recommendation.nombre} />
              </div>
            ...
            <p className="font-bold" onClick={handleLinkClick}>
              <span className="text-orange-600">Perfil:</span> <a href={recommendation.perfil}>{recommendation.perfil}</a>
            </p>
          </div>
        </div>
      ))}
